#!/usr/bin/env python3
"""
threshold_predictor.py  —  RegionSplit 最適閾値予測器 v3.1
====================================================================
...
def predict_threshold(
    ...
    dict with keys:
      ...
      path            : str          判定経路 ("ohw_jump" / "bulk" / "vdip" / "cdf_cross" / "neutral")
    ...
    # ──────────────────────────────────────────────────────────────
    # Path D: OHW V字谷検出
    #   有意ビン i において r[i] < r[i-1] かつ r[i] < r[i+1] (V字形)
    #   かつ N_b[i] が前後ビン平均の VDIP_N_RATIO 倍以上
    #   → このビンを大プールに残す (T = ビン i の下端)
    # ──────────────────────────────────────────────────────────────
    for i in range(1, len(sig) - 1):
        ...
        drop_left  = r_prev - r_curr   # 正 → 前より低い
        drop_right = r_next - r_curr   # 正 → 後より低い

        if drop_left >= VDIP_R_DROP and drop_right >= VDIP_R_DROP:
            n_curr = b_curr.get("req_count", 0)
            n_prev = b_prev.get("req_count", 0)
            n_next = b_next.get("req_count", 0)
            n_neighbor = (n_prev + n_next) / 2 if (n_prev + n_next) > 0 else 1
            ...
            if n_ratio >= VDIP_N_RATIO:
                ...
                advice = _advice_vdip(b_curr, n_ratio, drop_left, drop_right)
                return _result(T, "vdip", expl, b_curr, advice)
...
def _advice_vdip(b: dict, n_ratio: float, drop_left: float, drop_right: float) -> str:
    r = b.get("ohw_rate", 0)
    return (
        f"OHW V字谷型: ビン ({b.get('label', '?')}) はr={r:.3f}の局所最小値を持ち、"
...
def main():
    parser = argparse.ArgumentParser(
        description="RegionSplit 閾値予測器 v3.1",
    ...
    parser.add_argument(
        ...
        help="分析対象の最大ビン番号 (例: 27 で bin27以下のみ使用。スコープ外ビン除外に使用)"
    )
